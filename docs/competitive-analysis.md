...
| **프로덕션 실적** | **0건** | 2.7억 TX 스캔^6 | 해킹 20건+ 차단^7 | 1.4M+ 시뮬레이션^8 | 미공개 |
...
|------|------|
| 심각도 | **치명적** |
| 현상 | 모든 케이스 스터디가 가정법("would have detected"). 실제 탐지 실적 0건. |
| 영향 | 보안 도구 시장에서 실적은 신뢰의 전제 조건이다. 잠재 사용자가 "이 도구가 실제로 무언가를 잡은 적이 있는가?"라고 물으면 답이 "아직 없다". |
| 대응 | (1) 과거 유명 해킹 트랜잭션을 Autopsy에서 리플레이하여 "did detect (on replay)" 결과 공개. (2) Holesky 테스트넷 14일 연속 운영으로 첫 실적 확보. |
| 시기 | 단기 (Q2-Q3 2026) |
